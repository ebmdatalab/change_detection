...
#    print(!is.first==Inf)
  ...
  if (saveplots_analysis){
    filename <- paste(fig_path_tis_analysis, results$name[i], ".pdf", sep="")
    wid <- 800
    hei <- 800
    #dev.new(width = wid, height = hei, unit = "px")
    pdf(filename,width = 7,height = 5)
    #par(mfrow=c(1,1))
    par(mgp=c(2.2,0.45,0), tcl=-0.4, mar=c(3.3,3.6,1.1,1.1))
    islstr.res$aux$y[islstr.res$aux$y == 99] <- NA
    plot(islstr.res$aux$y, col="black", ylab="Proporiton of Cerazette", xlab="Time series months", type="l",ylim=c(0, 1),las=1) ##
