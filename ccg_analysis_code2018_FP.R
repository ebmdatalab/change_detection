...
###########################################################################
###########################################################################
#### Code to Detect and Classify Trend Breaks in CCG Prescription Data
###########################################################################
###########################################################################

rm(list = ls())  #clear the workspace
setwd("C:/Users/ajwalker/Documents/GitHub/prescribing_changes_response_metrics") ### Set working directory
#setwd("C:/Users/EMoD/OneDrive/Documents/Projects/Medical Practice/Data/CCG Analysis Sept 2018")
...
saveplots <- FALSE ###save plots of output
plot_show <- FALSE ###show plots during break detection

fig_path_tis <- paste("./figures/new2019/") ### set path to store break detection figures
...
data.pick <- read.csv("data.pick.csv", header=TRUE)   #load raw input data

#############################################
############################################

pick.rel <- grep(variable , names(data.pick)) 
mean <- data.frame(rowMeans(data.pick[pick.rel]))

colnames(mean) <- paste(variable, ".mean", sep="") #mean variable in dataset
month.c  <-  as.POSIXct(as.numeric(as.character(data.pick$month)),origin="1970-01-01",tz="GMT")

data.pick <- data.frame(month.c, mean, data.pick[,pick.rel]) 
...
withCallingHandlers({
...

  {
    ...
    if (saveplots){
      filename <- paste(fig_path_tis, names.rel[i], ".png", sep="")
      wid <- 800
      hei <- 900
...
########Save Break Detection Results
save(result.list, file="resultlistCCG_out.RData")
...
fig_path_tis_analysis <- paste("./figures/slopemeasure/") ###set path to store analysis figures
   ...
   if (NCOL(islstr.res$aux$mX[,tnames]) > 1){
    ...
    if (!is.first.neg.pknown==Inf)  #first break not to lie before the known break date
    {
      ...
      if (saveplots_analysis){
        ...
        hei <- 500
        par(mfrow=c(1,1))
        plot(islstr.res$aux$y, col="black", ylim=c(0, 1), ylab="Proportion of Cerazette", xlab="Time series months", type="l")
